...
export default function ReportFound() {
  ...
  const [form, setForm] = useState({
    name: "", description: "", date: "", location: "", category: "Other", imageFile: null
  })
  ...
  async function submit(e) {
    ...
    const { data: m, error: rpcError } = await supabase.rpc('smart_match_for_item', {
  ...
  return (
    <div className="max-w-2xl card p-6 mx-auto">
      ...
      <form className="grid gap-3" onSubmit={submit}>
        ...
        <textarea required placeholder="Where/when it was found" className="border px-3 py-2 rounded-xl"
          value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} />

        <input required type="date" className="border px-3 py-2 rounded-xl"
          value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} />

        <input required placeholder="Found location" className="border px-3 py-2 rounded-xl"
        ...
        <div className="fixed inset-0 bg-black/40 grid place-items-center p-4 z-50">
          <div className="card p-6 max-w-2xl bg-white">
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button className="btn btn-primary" onClick={() => { setShowMatches(false); nav('/found') }}>Continue</button>
            </div>
          </div>
        </div>
      )}
